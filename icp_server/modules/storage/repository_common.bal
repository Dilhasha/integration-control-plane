...
public isolated function issueRuntimeHmacToken(string runtimeId) returns string|error {
    ...
    ParsedSecret parsed = check parseHmacSecret(hmacSecret);
    ...
    jwt:IssuerConfig issConfig = {
        ...
        signatureConfig: {algorithm: jwt:HS256, config: parsed.keyMaterial}
    };
    issConfig.customClaims["scope"] = "runtime_agent";

    // Add keyId to the JWT header (as per RFC 7515)
    issConfig.keyId = parsed.keyId;
