-- Migration: stateless workflow command tunnel (H2)
...
-- 3. When a user is actively working with workflow views for a runtime's scope, the ICP
--    asks that runtime to heartbeat faster so queued work is picked up in about a second
--    instead of on its normal interval. Epoch seconds; NULL or past means no boost.
