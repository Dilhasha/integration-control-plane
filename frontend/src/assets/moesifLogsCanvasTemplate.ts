...
// including the `runtimeId` context filter (on the `resource.icp.runtimeId`
// OTLP resource attribute the Fluent Bit sidecars tag records with) whose options
// MoesifCanvas fills in with the integration's runtimes. Note the naming: logs
// carry the id as `resource.icp.runtimeId`, while the metrics canvas filters on
// the runtime-published metric tag `metadata.metric_tags.icp_runtimeid`.
